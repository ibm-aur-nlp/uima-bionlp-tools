 ...
 *
 */
public class FilterMutationDiseaseRelation {
		...
		for (File WDDFile : WDDFolder.listFiles()) {
			...
			if (WDDFile.getName().endsWith(".txt")) {
				...
				for (Entity e : d.getEntities()) {
					...
					// If no mutation or disease, then remove
					else if (!e.getType().equals("Disease")) {
						removeList.add(e);
					}
				}
				...
				// Remove any relation that is not a relation between disease and
				// mutation
				List<Relation> removeRelation = new ArrayList<>();

				for (Relation r : d.getRelations()) {
					if (!(r.getEntity1().getType().equals("Disease")
